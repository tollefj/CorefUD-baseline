...
if __name__ == "__main__":
    ...
    # check a third argument for whether to generate a test set (holdout)
    make_test = False
    if len(sys.argv) > 3:
        make_test = sys.argv[3]
    test_ratio = 0.1
    holdout_path = out_path.replace(".jsonl", "_holdout.jsonl")
    holdout_docs = {}

    with open(in_path, encoding="utf-8") as f:
        grouped_docs = get_grouped_docs(conllu.parse(f.read()))
        # make a test set of 10% of grouped docs
        if make_test:
            # select a random subset of the keys
            # set seed:
            random.seed(42)
            random_keys = random.sample(grouped_docs.keys(), int(len(grouped_docs) * test_ratio))
            rest = set(grouped_docs.keys()) - set(random_keys)

            holdout_docs = {k: grouped_docs[k] for k in random_keys}
            grouped_docs = {k: grouped_docs[k] for k in rest}

        # extract the filename from in_path:
        ...
        with jsonlines.open(out_path, mode="w") as writer:
            for part_id, _doc in enumerate(grouped_docs.values()):
                ...
                writer.write(parsed)
        if make_test:
            with jsonlines.open(holdout_path, mode="w") as writer:
                for _doc in holdout_docs.values():
                    parsed = parse_doc(_doc)
                    writer.write(parsed)
